      ...
      <form @submit.prevent="submitForm" class="field-form">
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn-submit" :disabled="loading">
            <span v-if="!loading">Update Field</span>
...
export default {
  ...
  data() {
    return {
      formData: {
        name: this.field.nom || '',
        type: this.field.sport || '',
        location: this.field.adresse || '',
        ville: this.field.ville || '',
        size: this.field.taille || null,
        horaires: this.field.horaires || '',
        prix: this.field.prix || null,
        description: this.field.description || '',
        image: this.field.image_url || ''
      },
      loading: false
    };
  },
  methods: {
    async submitForm() {
      ...
      try {
        const response = await fetch(`http://localhost:3000/fields/${this.field.id}`, {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify({
            ...
            sport: this.formData.type,
            adresse: this.formData.location,
            ville: this.formData.ville,
            taille: this.formData.size,
            horaires: this.formData.horaires,
            prix: this.formData.prix,
            description: this.formData.description,
            image_url: this.formData.image || null
          })
        });
